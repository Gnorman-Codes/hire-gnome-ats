...
import { getIntegrationSettings } from './system-settings.js';
...
async function pruneEmptyLocalDirectories(filePath, localRoot) {
	const root = path.resolve(localRoot);
	let currentDirectory = path.dirname(filePath);

	while (currentDirectory !== root && currentDirectory.startsWith(`${root}${path.sep}`)) {
		try {
...
export async function deleteObject({ key, storageProvider, storageBucket }) {
	...
	if (useLocal) {
		...
		await pruneEmptyLocalDirectories(absolutePath, config.localRoot);
		return;
	}
